...
| Digital | A combinational loop never settles: the event queue evaluates a gate when an input changes, and it does not iterate to a fixed point | Cross-couple two NOR gates into an SR latch and the pair sits at unknown forever, whatever the inputs do. NOR(1, X) resolves to 0 by dominance, so one side does settle, but NOR(0, X) is X and there is no second pass to feed the settled side back. An SR latch example was written and dropped over this. The flip-flop part sidesteps it by powering up low rather than unknown — see the comment in `DFlipFlop::new` — and a latch built out of gates has no such escape. Fixing it means a settle pass at t = 0 with an iteration cap, in the engine |
