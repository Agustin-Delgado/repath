<script lang="ts">
	 ...
	 */
	import { Play, RotateCcw, StepForward } from '@lucide/svelte';
...
<div
	class="flex items-center gap-1.5 border-b border-border bg-panel px-2.5 py-1 text-[0.75rem] max-[900px]:overflow-x-auto max-[900px]:[scrollbar-width:none] max-[900px]:*:shrink-0"
>
	...
	<div class="flex" role="group" aria-label="Sweep speed">
		{#each SPEEDS as speed, i (speed)}
			<Button
				...
				class="h-6.5 px-1.5 font-mono {i > 0 ? '-ml-px rounded-l-none' : ''} {i < SPEEDS.length - 1
					? 'rounded-r-none'
					: ''}"
			>
